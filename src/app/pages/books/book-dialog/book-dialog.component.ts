...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, MatDialogModule, MatFormFieldModule, MatInputModule, MatButtonModule, MatIconModule, UploadFileFormComponent, MatSlideToggleModule, MatProgressSpinnerModule, MatTooltipModule],
...
export class BookDialogComponent {
  ...
  bookForm = this.formBuilder.nonNullable.group({
    ...
    publicationYear: new FormControl<number | null>(this.bookDialogData.book?.publicationYear ?? null, Validators.required),
    publicationPlace: [this.bookDialogData.book?.publicationPlace ?? '', [Validators.required]],
    pages: new FormControl<number | null>(this.bookDialogData.book?.pages ?? null, [Validators.required, Validators.min(1)]),
